...
const CF = require('../utils/ContainerFormatter');
...
class ReportChatSystem {
    ...
    createBaseContainer(guild, reportNumber, user, status = 'waiting', staffs = [], extraDescription = '') {
        ...
        if (status === 'closed_no_reason' || status === 'closed_with_reason') {
            color = CF.colors.info;
        } else if (status === 'responded') {
            color = CF.colors.success;
        } else {
            color = CF.colors.error;
        }

        const builder = CF.create({ color, serverName: guild.name });
        const reportIdDisplay = `#R${reportNumber}`;

        // ── 1. HEADER com avatar thumbnail ───────────────────────────────────
        builder.sectionWithThumb(
            [
        ...
        let closedAt = null;
        const closedReason = reportInfo?.closed_reason || null;
        const punishment   = reportInfo?.punishment    || null;

        if (reportInfo?.closed_by) {
            try {
        ...
        if (reportInfo?.thread_id) {
            ...
            builder.sectionWithButton(
                statusText,
                CF.button(threadLink, '🔗 Ir para o chat', 'link'),
            );
        } else {
        ...
        // ── 4. STAFFS ────────────────────────────────────────────────────────
        if (staffs?.length > 0) {
            const staffLines = staffs.map(s => {
                const entryTime = `<t:${Math.floor(s.timestamp / 1000)}:R>`;
                return `<@${s.id}> (entrou ${entryTime})`;
            });
            builder.text(`### 👥 Staffs:\n${staffLines.join('\n')}`);
            builder.line();
        }
        ...
        // ── 5. AVALIAÇÃO ─────────────────────────────────────────────────────
        if (reportInfo?.rating > 0) {
            builder.text(
                [
                    `### ⭐ Avaliação: ${reportInfo.rating}/5`,
                    reportInfo.rating_comment ? `\`\`\`${reportInfo.rating_comment}\`\`\`` : null,
                    CF.stars(reportInfo.rating),
                ].filter(Boolean).join('\n'),
            );
            builder.line();
        }
    ...
    getCloseModalUser() {
        ...
        modal.addComponents(
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('motivo').setLabel('Qual motivo do fechamento?').setStyle(TextInputStyle.Short).setRequired(true).setPlaceholder('Ex: Problema resolvido')),
        );
        return modal;
    }
    ...
    getPanel(guildName, guildIcon) {
        const container = CF.create({ color: 0xDCA15E, serverName: guildName })
            .title(`${EMOJIS.chat || '🎫'} Denúncia de jogador`, 1)
            .text([
        ...
        return {
            components: [container, new ActionRowBuilder().addComponents(reportButton)],
    ...
    async openReport(interaction, data) {
        ...
        try {
            ...
            // ── Container de boas-vindas da thread ───────────────────────────
            const threadContainer = CF.create({ color: 0xDCA15E, serverName: guild.name })
                .title(`${EMOJIS.chat || '🗨️'} REPORTE | ${reportId}`, 1)
                .text(`Obrigado por abrir o reporte. Um membro da staff irá te atender em breve.\n\nEnquanto aguarda, você pode adicionar mais informações ou provas neste chat.`)
                .footer()
                .done();
            ...
            const infoContainer = CF.create({ color: 0xDCA15E, serverName: guild.name })
                .title(`${EMOJIS.chat || '📋'} Informações do Report`, 1)
                .line()
                .text(`**📝 Regra quebrada:** ${data.regra}`)
                .text(`**⏰ Quando aconteceu:** ${data.dataHora}`)
                .text(`**📍 Local:** ${data.local || 'Não informado'}`)
                .text(`**📋 Descrição:** ${data.descricao}`)
                .text(`**⚖️ Termo de convivência:** ${data.termo}`)
                .footer()
                .done();
            ...
            dmBuilder.buttons(
                new ButtonBuilder().setCustomId(`close:${guild.id}:${reportNumber}`).setLabel('Fechar').setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId(`close_reason:${guild.id}:${reportNumber}`).setLabel('Fechar com Motivo').setStyle(ButtonStyle.Primary),
            );

            const dmMessage = await user.send({
                components: [dmBuilder.done()],
                flags: MessageFlags.IsComponentsV2,
            }).catch(() => null);
            ...
            logBuilder.buttons(
                new ButtonBuilder().setCustomId(`join:${reportId}`).setLabel('Entrar no Reporte').setStyle(ButtonStyle.Success),
                new ButtonBuilder().setCustomId(`close:${reportId}`).setLabel('Fechar').setStyle(ButtonStyle.Danger),
                new ButtonBuilder().setCustomId(`close_reason:${reportId}`).setLabel('Fechar com Motivo').setStyle(ButtonStyle.Primary),
            );

            const logMessage = await logChannel.send({
                components: [logBuilder.done()],
                flags: MessageFlags.IsComponentsV2,
            });
        ...
        try {
            ...
            if (logChannelId && report.log_message_id) {
                ...
                if (logMessage) {
                    ...
                    const buttonsToPreserve = logMessage.components.slice(1);
                    await logMessage.edit({
                        components: [updatedBuilder.done(), ...buttonsToPreserve],
                        flags: MessageFlags.IsComponentsV2,
                    });
                }
            }
            ...
            if (report.dm_message_id) {
                ...
                if (dmMessage) {
                    const updatedBuilder = this.createBaseContainer(guild, reportNumber, targetUser, report.status, staffs);
                    const buttonsToPreserve = dmMessage.components.slice(1);
    ...
    async closeReport(interaction, reportNumber, motivo, punicao, hasReason, guildId = null) {
        try {
            ...
            if (!report) {
                await this.sendTempReply(interaction, `Report #R${reportNumber} não encontrado.`, false);
                return;
            }
            ...
            if (thread) {
                await thread.send({ content: `🔒 Report fechado por ${closedByMention}` }).catch(() => {});
            ...
            if (logChannelId && report.log_message_id) {
                try {
                    ...
                    const logMessage = await logChannel.messages.fetch(report.log_message_id);
                    if (logMessage) {
                        const updatedContainer = this.createBaseContainer(guild, reportNumber, targetUser, status, staffs).done();
                        await logMessage.edit({ components: [updatedContainer], flags: MessageFlags.IsComponentsV2 });
                    }
                } catch {}
            }

            // ── Atualizar DM do usuário com botão de avaliação ────────────────
            if (report.dm_message_id) {
                try {
                    ...
                    if (dmMessage) {
                        ...
                        updatedBuilder.buttons(
                            new ButtonBuilder()
                                .setCustomId(`rate:${guild.id}:${reportNumber}`)
                                .setLabel('Avaliar Atendimento')
                                .setStyle(ButtonStyle.Secondary),
                        );
                        await dmMessage.edit({ components: [updatedBuilder.done()], flags: MessageFlags.IsComponentsV2 });
                    }
                } catch {}
            }
    ...
    async rateReport(interaction, reportNumber, nota, comentario, guildId = null) {
        try {
            ...
            // ── Atualizar log da staff com avaliação ──────────────────────────
            ...
            if (logChannelId && report.log_message_id && guild) {
                try {
                    ...
                    const logMessage = await logChannel.messages.fetch(report.log_message_id);
                    if (logMessage) {
                        const updatedContainer = this.createBaseContainer(guild, reportNumber, targetUser, report.status, staffs).done();
                        await logMessage.edit({ components: [updatedContainer], flags: MessageFlags.IsComponentsV2 });
                    }
                } catch {}
            }
    ...
    async sendTempReply(interaction, content, success = true) {
        ...
        if (interaction.replied || interaction.deferred) {
            await interaction.editReply({ content: `${emoji} ${content}`, flags: MessageFlags.Ephemeral });
        } else {
            await interaction.reply({ content: `${emoji} ${content}`, flags: MessageFlags.Ephemeral });
        }
