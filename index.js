...
let handler = null;
let isReady = false;
let autoModInstance = null; // Adicionar variável para manter referência
...
module.exports = {
    ...
    async execute(client) {
        ...
        // 7. Iniciar AutoModeração Worker - CORRIGIDO
        try {
            // Verificar se já existe uma instância global
            if (!global.autoModInstance) {
                // Importar e inicializar apenas uma vez
                const autoModeration = require('../systems/autoModeration');
                autoModInstance = autoModeration(client);
                console.log(`🛡️ AutoModeração Worker iniciado automaticamente`);
                console.log(`🛡️ Status: ${autoModInstance.isRunning ? 'Rodando' : 'Parado'}`);
            } else {
                console.log(`🛡️ AutoModeração Worker já estava rodando (reutilizando instância)`);
                autoModInstance = global.autoModInstance;
                console.log(`🛡️ Status: ${autoModInstance.isRunning ? 'Rodando' : 'Parado'}`);
            }
            
            // NÃO CHAMAR startWorker() novamente - já foi iniciado automaticamente
            // autoMod.startWorker(); // ← REMOVER ESTA LINHA
